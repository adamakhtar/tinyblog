    ...
    window.inlineAttach = function(options, instance) {
        ...
        this.onUploadedFile = function(data) {
            ...
            if (result !== false && filename) {
                var urlText = settings.urlText.replace(filenameTag, filename);
                var text = editor.getValue().replace(lastValue, urlText);
                editor.setValue(text);
            }
        };

        /**
         * Returns basename of a file given it's path
         *
         * @param {Blob} file
            ...
            if (typeof clipboardData === "object") {
                ...
                for (var i = 0; i < items.length; i++) {
                    ...
                    if (me.isAllowedFile(item)) {
                        result = true;
                        this.onReceivedFile(item.getAsFile(), pos);
        ...
        this.onDrop = function(e) {
            ...
            var result = false,
                pos    = editor.coordsChar({left: e.pageX, top: e.pageY});

            for (var i = 0; i < e.dataTransfer.files.length; i++) {
                var file = e.dataTransfer.files[i];
                if (me.isAllowedFile(file)) {
                    result = true;
                    this.onReceivedFile(file, pos);
...
(function(document, window) {
    ...
    function CodeMirrorEditor(instance) {
        ...
        return {
            ...
            insertValue: function(val, pos){
                codeMirror.replaceRange(val, pos);
                codeMirror.setCursor(pos);
            },

            coordsChar: function(coords){
                return codeMirror.coordsChar(coords);
            },
    ...
    window.inlineAttach.attachToCodeMirror = function(codeMirror, options) {
        ...
        // Adam: This no longer works in the current version (4) - use handler above.
